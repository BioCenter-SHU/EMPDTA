...
sweep_configuration = {
    ...
    'parameters':
    {
        ...
        'num_epoch': {'values': [120]},
        'batch_size': {'values': [16]},
        # 'fold': {'values': [0, 1, 2]},  # , 1, 2, 3, 4
        'learning_rate': {'values': [1e-3]},
        'weight_decay': {'values': [1e-3]},
        'target_readout': {'values': ["mean"]},
        'target_short': {'values': [False]},
        'target_concat': {'values': [False]},
        'target_hidden': {'values': [256]},
        'target_layer': {'values': [4]},
        'drug_readout': {'values': ["sum"]},
        'drug_short': {'values': [False]},
        'drug_concat': {'values': [False]},
        'drug_hidden': {'values': [128]},
        'drug_layer': {'values': [3]},
        'point_embedding': {'values': [16]},
        'point_layer': {'values': [3]},
        'point_topk': {'values': [32, 64, 128, 256]},  # 32, 64, 128, 256
        'point_radius': {'values': [6.0, 9.0, 12.0, 15.0]},  # 6.0, 9.0, 12.0, 15.0
        'loss_weight': {'values': [0.5]}
    }
}

sweep_id = wandb.sweep(sweep=sweep_configuration, project='AIO PDBbind Sweep')
# sweep_id = "marinehdk/AIO PDBbind Sweep/int2fng2"
...
def main():
    ...
    dataset = datasets.PDBBind(protein_method="gearnetesm", drug_method="distanceMol", transform=transform, lazy=True)
    ...
    task = tasks.AffinityWithDocking(
        ...
        task=dataset.tasks, criterion="mse",
        criterion_weight={"Affinity MSE": 1, "Pocket BCE": wandb.config.loss_weight},
        metric=("rmse", "mse", "c_index", "pearsonr", "spearmanr", "rm2", "mae", "macro_auroc", "macro_auprc")
    )
    task.apply(initialize_weights)

    # ============================== Pretrained params loading for both models ============================== #
    # model_checkpoint = "../../result/model_pth/POD_Pretrain_PDBbind.pth"
    ...
    checkpoint = "../../result/model_pth/MD_AIO_Sweep_PDBbind.pth"
...
wandb.agent(sweep_id=sweep_id, function=main, count=16)
