...
sweep_id = wandb.sweep(sweep=sweep_configuration, project='AIO Davis Sweep')
# sweep_id = "marinehdk/AIO Davis Sweep/wnnhwtai"
...
def main():
    ...
    dataset = datasets.Davis(protein_method="gearnetesm650m", drug_method="distanceMol", transform=transform)
    ...
    task = tasks.AffinityWithDocking(
        ...
        metric=("rmse", "mse", "c_index", "pearsonr", "spearmanr", "rm2", "macro_auroc", "macro_auprc")
    )
    task.apply(initialize_weights)

    # ============================== Pretrained params loading for both models ============================== #
    # model_checkpoint = "../../result/model_pth/POD_Pretrain_Davis.pth"
    model_checkpoint = "../../result/model_pth/PD_Sweep_PDBbind_R" + str(wandb.config.point_radius) + ".pth"
