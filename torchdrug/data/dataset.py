...
class ProteinLigandDataset(ProteinDataset, core.Configurable):
    ...
    Some function should be added for DTA, like pdb about method for 3D NN to encoder the point positon.
    """
    def load_drug(self, drug_file, drug_method, **kwargs):
        """
        Load the distinct drugs into a list from dataset.
        Parameters:
            drug_file (str): distinct drug csv file path.
            drug_method (str): drug loading method 'smile', '2d' or '3d'.
            **kwargs
        """
        drug_df = pd.read_csv(drug_file)
        drug_list = []  
        # Generate the Protein one by one
        if drug_method == 'smile':
            smiles = drug_df["SMILES"].tolist() 
            smiles = tqdm(smiles, "Constructing Drug from Smiles: ")
            for smile in smiles:
                mol = Chem.MolFromSmiles(smile)
                if not mol:
                    logger.debug("Can't construct molecule from SMILES `%s`. Ignore this sample." % smile)
                    continue
                mol = data.Molecule.from_molecule(mol, mol_feature='default') 
                drug_list.append(mol)
        elif drug_method == '2d':
            smiles = drug_df["SMILES"].tolist() 
            smiles = tqdm(smiles, "Constructing Drug from Smiles with 2D atom position: ")
            for smile in smiles:
                mol = Chem.MolFromSmiles(smile)
                if not mol:
                    logger.debug("Can't construct molecule from SMILES `%s`. Ignore this sample." % smile)
                ...
                d = data.Molecule.from_molecule(mol, mol_feature='default') 
                with d.node():
                    d.node_position = torch.tensor([data.feature.atom_position(atom) for atom in mol.GetAtoms()])
                drug_list.append(d)
        elif drug_method == '3d':
            smiles = drug_df["SMILES"].tolist() 
            smiles = tqdm(smiles, "Constructing Drug from Smiles with 3D atom position: ")
            for smile in smiles:
                mol = Chem.MolFromSmiles(smile)
                mol = Chem.AddHs(mol)
                AllChem.EmbedMolecule(mol, randomSeed=42)  # `useRandomCoords=True` if error occur, use this code
                Chem.rdForceFieldHelpers.MMFFOptimizeMolecule(mol)
                mol = Chem.RemoveHs(mol)
                d = data.Molecule.from_molecule(mol, atom_feature='default' , bond_feature='length', mol_feature='default')
                with d.node():
                    d.node_position = torch.tensor([data.feature.atom_position(atom) for atom in mol.GetAtoms()])
                if not mol:
                    logger.debug("Can't construct molecule from SMILES `%s`. Ignore this sample." % smile)
                    continue
                drug_list.append(d)
        drug_pkl = self.path + drug_method + '_Molecule.pkl'
        with utils.smart_open(drug_pkl, "wb") as fout:
            pickle.dump(drug_list, fout)
            # ============================== PKL Completing! ============================== #

    def load_protein(self, protein_file, protein_method, **kwargs):
        """
        Load the distinct proteins into a list from dataset.
        Parameters:
            protein_file (str): distinct protein csv file path.
            protein_method (str): Protein loading method 'sequence' or 'pdb' or 'pocket'.
            **kwargs
        """
        # if protein_method == 'pocket':
        #     protein_file = self.path + "kiba_pocket_proteins.csv"
        protein_df = pd.read_csv(protein_file)
        protein_list = []  
        # Generate the Protein one by one
        if protein_method == 'sequence':
            sequences = protein_df["Sequence"].tolist() 
            sequences = tqdm(sequences, "Constructing Protein from Sequences: ")
            for sequence in sequences:
                protein = data.Protein.from_sequence(sequence, **kwargs) 
                protein_list.append(protein)

        elif protein_method == 'pdb':
            pdbs = protein_df["PDBID"].tolist() 
            pdbs = tqdm(pdbs, "Constructing Protein from PDB: ")
            for pdb in pdbs:
                # alphafold 3d pdb format 'AF-O06917-F1-model_v3.pdb'
                pdb_name = self.path + 'pdb/AF-' + pdb + '-F1-model_v4.pdb'
                protein = data.Protein.from_pdb(pdb_name, **kwargs)
                protein_list.append(protein)
        
        elif protein_method == 'pocket':
            protein_df = pd.read_csv(protein_file)
            pdbs = protein_df["PDBID"].tolist() 
            pdbs = tqdm(pdbs, "Constructing Protein from Pocket: ")
            i = 0
            protein_list = []
            for pdb in pdbs:
                # alphafold 3d pdb format 'AF-O06917-F1-model_v3.pdb'
                pdb_file = path + 'pdb/AF-' + pdb + '-F1-model_v4.pdb'
                protein = data.Protein.from_pdb(pdb_file)
                # get the pocket number
                pocket_num = protein_df.loc[i]["Pocket_No"]
                # get all the pockets from each pdb 
                pocket_index_list = protein_df.loc[i]["Pocket_Index"]
                # convert the string into list by the eval() func
                pocket_index_list = eval(pocket_index_list)
                # get whole protein for the Pocket_Probability=1.0
                if pocket_num==1 and protein_df.loc[i]["Pocket_Probability"][0] == 1.0:
                    protein_list.append([protein])
                else:
                    current_pocket_list = []
                    # get the pocket list to generate all the pocket
                    for j in range(pocket_num):
                        current_pocket_list.extend(pocket_index_list[j])
                    # delete the repeat items from the list using `list(set(mylist))`
                    pocket = protein.subresidue(index=list(set(current_pocket_list)))  
                    protein_list.append(pocket)
                i += 1

        protein_pkl = self.path + protein_method + '_Protein.pkl'
        with utils.smart_open(protein_pkl, "wb") as fout:
            pickle.dump(protein_list, fout)
            # ============================== PKL Completing! ============================== #

    # added for pdb pkl loading
    def load_pickle(self, pkl_file, transform=None, lazy=False, verbose=0, **kwargs):
        """
        Load the dataset from a pickle file.
        ...
        with utils.smart_open(pkl_file, "rb") as fin:
            ...
            if verbose:
                indexes = tqdm(indexes, "Loading %s" % pkl_file)
            for i in indexes:
                pdb_file, sequence, smile, target, data  = pickle.load(fin)
                self.pdb_files.append(pdb_file)
                self.sequences.append(sequence)
                self.smiles.append(smile)
                self.targets['affinity'].append(target)
                self.data.append(data)

    # added for pdb pkl storing
    def save_pickle(self, pkl_file, verbose=1):
        with utils.smart_open(pkl_file, "wb") as fout:
            num_sample = len(self.data)
            pickle.dump(num_sample, fout)
            ...
            if verbose:
                indexes = tqdm(indexes, "Dumping to %s" % pkl_file)
            for i in indexes:
                pickle.dump((self.pdb_files[i], self.sequences[i], self.smiles[i], self.targets['affinity'][i], self.data[i]), fout)
