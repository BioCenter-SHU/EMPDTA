...
class SpatialGraphConv(MessagePassingBase):
    """
    Spatial Graph convolution operator from `Spatial Graph convolution Networks`.
    ...
    def __init__(self, input_dim, output_dim, filter_num=64, batch_norm=False, activation="relu"):
        ...
        self.linear_in = nn.Linear(3, input_dim * filter_num)
        self.linear_out = nn.Linear(input_dim * filter_num, output_dim)

    def message(self, graph, input):
        node_in, node_out, relation = graph.edge_list.t()
        relative_pos = graph.node_position[node_out] - graph.node_position[node_in]  # [edge_num, 3]
        # add self loop
        node_in = torch.cat([graph.edge_list[:, 0], torch.arange(graph.num_node, device=graph.device)])  #  [edge_num + node_num]
        relative_pos = torch.cat([relative_pos, torch.zeros([graph.num_node, 3], device=graph.device)], dim=0)  #  [edge_num + node_num, 3]
        spatial_scaling = F.relu(self.linear_in(relative_pos))  # [edge_num + node_num, input_dim * filter_num]
        message = (spatial_scaling.reshape(graph.num_edge + graph.num_node, self.input_dim, -1) *
                   input[node_in].unsqueeze(-1))  #  [edge_num + node_num, input_dim, filter_num]
        message = message.view(graph.num_edge + graph.num_node, -1)  # [edge_num + node_num, input_dim, filter_num]
        return message
...
class EquivariantRelationalGraphConv(MessagePassingBase):
    """
    我在 EGNN 上针对分子提出的ERGNN就是增强了不同边附不同重要程度分开传递这一特征。
    Equivariant Graph Convolutional Layer from `E(n) Equivariant Graph Neural Networks
    <https://arxiv.org/abs/2102.09844>`_.

        .. math:: m_{ij}=\phi_e(h_i^l, h_j^l, ||x_i^l-x_j^l||^2, a_{ij})    (1)
        .. math:: x_i^{l+1} = x_i^l + C\sum_{j\in\mathcal{N}(i)}(x_i^l-x_j^l)\phi_x(m_{ij})    (2)
        .. math:: m_i = \sum_{j\in\mathcal{N}(i)} m_{ij}    (3)
        .. math:: h_i^{l+1} = \phi_h(h_i^l, m_i)   (4)
    ...
    # ========== TODO: 将药物坐标进行更新 ========== #
    def __init__(self, input_dim, hidden_dim, output_dim, num_relation, edge_input_dim=19,
                 batch_norm=False, activation="silu", dropout=0.2):
        super(EquivariantRelationalGraphConv, self).__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
        self.num_relation = num_relation
        ...
        self.edge_mlp = nn.Sequential(
            nn.Linear(input_dim * 2 + edge_input_dim + 1, hidden_dim),
            nn.SiLU(),
        ...
        self.node_mlp = nn.Sequential(
            nn.Linear(input_dim + hidden_dim * num_relation, hidden_dim),
            nn.SiLU(),
    ...
    def message(self, graph, input):
        ...
        Parameters:
            ...
            input (Tensor): node representations of shape :math:`(|V|, ...)`
        Returns:
            edge_message: edge messages of shape :math:`(|E|, ...)`
        """
        # ============================== Eq.3 Message Stage ============================== #
        # ====== Edge Section: cat infos from node i, node j, edge ij and dist between ij ====== #
        # ====== node_in: j, node_out: i, relation: bond type ====== #
        node_in, node_out, relation = graph.edge_list.t()
        hidden_j = input[node_in]  # [num_edges, node_feat_dim]
        hidden_i = input[node_out]  # [num_edges, node_feat_dim]
        r_ij = graph.node_position[node_out] - graph.node_position[node_in]  # [edge_num, 3]
        radial_ij = r_ij.square().sum(dim=1).unsqueeze(-1)
        edge_message = torch.cat([hidden_i, hidden_j, radial_ij, graph.edge_feature], dim=-1)
        # ============================== Eq.4 Coords Stage ============================== #
        m_ij = self.edge_mlp(edge_message)  # [num_edges, hidden_dim]
        coord_message = self.coord_linear(m_ij)   # [num_edges, 1]
        r_ij = r_ij / (radial_ij.sqrt() + self.eps)   # [num_edges, 3]
        constant_c = (graph.num_nodes[graph.atom2graph[node_out]] - 1).unsqueeze(dim=-1)   # [num_edges, 1]
        x_fusion = coord_message * r_ij / constant_c  # [num_edges, 3]
        coord_update = scatter_add(x_fusion, node_out, dim=0, dim_size=graph.num_node)  # [num_nodes, 3]
        original_coord = graph.node_position
        updated_coord = original_coord + coord_update
        graph.node_position = updated_coord  # update the position for the molecule
    ...
    def aggregate(self, graph, message):
        ...
        Parameters:
            graph (Graph): graph(s)
            message: edge messages of shape :math:`(|E|, ...)`
        Returns:
            node_update: node updates of shape :math:`(|V|, ...)`
        """
        assert graph.num_relation == self.num_relation
        ...
        update = scatter_add(message * edge_weight, node_out, dim=0, dim_size=graph.num_node * self.num_relation) / \
                 (scatter_add(edge_weight, node_out, dim=0, dim_size=graph.num_node * self.num_relation) + self.eps)
        return update.view(graph.num_node, self.num_relation * self.hidden_dim)  # node_update
    ...
    def combine(self, input, update):
        ...
        Parameters:
            ...
            update (Tensor): node updates of shape :math:`(|V|, ...)`
        Returns:
            output: node updates of shape :math:`(|V|, ...)`
        """
        output = self.node_mlp(torch.cat([input, update], dim=-1))
        if self.batch_norm:
            output = self.batch_norm(output)
        if self.dropout:
...
class PointNetConv(MessagePassingBase):
    def __init__(self, input_dim, output_dim, batch_norm=False, activation="relu"):
        ...
        self.edge_mlp = layers.MLP(input_dim + 3, [output_dim] * 2)

    def message(self, graph, input):
        node_in, node_out = graph.edge_list.t()
        hidden_j = input[node_in]  # [num_edges, node_feat_dim]
        position_i = graph.node_position[node_out] # [edge_num, 3]
        position_j =  graph.node_position[node_in]
        edge_message = torch.cat([hidden_j, position_j - position_i], dim=-1)
        message = self.edge_mlp(edge_message)
        return message
