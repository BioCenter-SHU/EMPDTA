...
class GraphConstruction(nn.Module, core.Configurable):
    ...
    Parameters:
        ...
        edge_feature (str, optional): edge features in the new graph.
            Available features are ``residue_type``, ``gearnet``, added ``comenet``.
    ...
    def edge_comenet(self, graph, edge_list):
        node_in, node_out, _ = edge_list.t()
        spatial_dist = (graph.node_position[node_in] - graph.node_position[node_out]).norm(dim=-1)
        graph.view = "atom"
        return torch.cat([
    ...
    def edge_molecule(self, graph, edge_list):
        node_in, node_out, _ = edge_list.t()
        # in_atom_type, out_atom_type = graph.atom_type[node_in], graph.atom_type[node_out]
        spatial_dist = (graph.node_position[node_in] - graph.node_position[node_out]).norm(dim=-1)
        graph.view = "atom"
        return torch.cat([
            # functional.one_hot(in_atom_type, 18), # 入边的原子类型
            # functional.one_hot(out_atom_type, 18), # 出边的原子类型
            spatial_dist.unsqueeze(-1)  # 边的空间距离就能说明不同的键类型，比如单键双键这种可以通过键长区分
        ], dim=-1)
    ...
    def apply_edge_layer(self, graph):
        ...
        elif self.edge_feature == "comenet":
            edge_feature = self.edge_comenet(graph, edge_list)
        elif self.edge_feature == "molecule":
            edge_feature = self.edge_molecule(graph, edge_list)
        else:
