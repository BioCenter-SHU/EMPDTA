...
class KNNEdge(nn.Module, core.Configurable):
    ...
    def __init__(self, k=10, min_distance=5, max_distance=None, mode="residues"):
    ...
    def forward(self, graph):
        ...
        if self.mode == "residues":
            ...
            if self.min_distance > 0:
                node_in, node_out = edge_list.t()[:2]
                mask = (graph.node_position[node_in] - graph.node_position[node_out]).norm(dim=-1) < self.min_distance
                edge_list = edge_list[~mask]
            if self.max_distance:
                node_in, node_out = edge_list.t()[:2]
                mask = (graph.node_position[node_in] - graph.node_position[node_out]).norm(dim=-1) > self.max_distance
                edge_list = edge_list[~mask]
...
class SpatialEdge(nn.Module, core.Configurable):
    ...
    def __init__(self, radius=5, min_distance=5, max_distance=None, max_num_neighbors=32, mode="residues"):
    ...
    def forward(self, graph):
        ...
        Returns:
            (Tensor, int): edge list of shape :math:`(|E|, 3)`, number of relations
        """
        edge_list = radius_graph(graph.node_position, r=self.radius, batch=graph.node2graph,
                                 max_num_neighbors=self.max_num_neighbors).t()
        relation = torch.zeros(len(edge_list), 1, dtype=torch.long, device=graph.device)
        edge_list = torch.cat([edge_list, relation], dim=-1)
...
@R.register("layers.geometry.PocketNode")
class PocketNode(nn.Module, core.Configurable):
    """
    Construct nodes from protein atoms by cutting off a spatial ball of the ligand center coords.

    Parameters:
        center_coords (tensor): the center coords of the corresponding ligand
        entity_level (str, optional): level to perform cropping. 
            Available options are ``node``, ``atom`` and ``residue``.
        min_radius (float, optional): minimum radius of the spatial ball
        min_neighbor (int, optional): minimum number of nodes in the spatial ball
    ...
    def __init__(self, entity_level="atom", min_radius=8.0, min_neighbor=16):
    ...
    def forward(self, graph):
        """
        Pick molecule center as the radius center, and crop a spatial ball
        that is at least `radius` large and contain at least `k` nodes.

        Parameters:
            graph (Graph): Protein graph
        """
        center_coords = graph.node_position[graph.atom_source == 0].mean(dim=0)  # 药物分子的中心坐标
        node_out = torch.arange(graph.num_node, device=graph.device)
        dist = (center_coords - graph.node_position[node_out]).norm(dim=-1)
        topk_dist = functional.variadic_topk(dist, graph.num_nodes, self.min_neighbor, largest=False)[0]
        radius = (topk_dist[:, -1] * 1.5).clamp(min=self.min_radius)
        radius = radius.repeat_interleave(graph.num_nodes)
        node_index = node_out[dist < radius]
