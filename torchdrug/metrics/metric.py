...
@R.register("metrics.rm2_index")
def rm2(pred, target):
    """
    :math:`R_m^2` regression score.

    Parameters:
        ys_line (Tensor): predictions of shape :math:`(n,)`
        ys_orig (Tensor): targets of shape :math:`(n,)`
    """
    ys_line = pred.cpu()
    ys_orig = target.cpu()
    r2 = r_squared_error(ys_orig, ys_line)
    r02 = squared_error_zero(ys_orig, ys_line)
    rm2_value= r2 * (1 - np.sqrt(np.absolute((r2 * r2)-(r02 * r02))))
    return torch.tensor(rm2_value, dtype=torch.float, device=pred.device)
# ========== Rm2 ========== #


@R.register("metrics.c_index")
def c_index(pred, target):
    """
    :math:`C-Index` regression score.

    Parameters:
        pred (Tensor): predictions of shape :math:`(n,)`
        target (Tensor): targets of shape :math:`(n,)`
    """
    return concordance_index(target.cpu(), pred.cpu())
