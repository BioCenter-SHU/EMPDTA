...
@R.register("transforms.TruncateMoleucle")
class TruncateMoleucle(core.Configurable):
    """
    Truncate over long drug smiles into a fixed length.
    ...
    def __call__(self, item):
        ...
        for key in self.keys:
            graph = item[key]
            if graph.num_atom > self.truncate_length:
                if self.random:
                    start = torch.randint(graph.num_atom - self.truncate_length, (1,)).item()
                else:
                    start = 0
                end = start + self.truncate_length
                mask = torch.zeros(graph.num_atom, dtype=torch.bool, device=graph.device)
                mask[start:end] = True
                graph = graph.node_mask(mask)
